...
//根据需要采集的点 fCoord 计算新的点(可能是需要改变为眼睛内部的点，完成放大的效果)
vec2 calcCoord(vec2 coord, vec2 eye, float rmax){
    vec2 newCoord = coord;
    //获得当前需要采集的点与眼睛的距离
...
void main(){
    ...
    // 如果属于 右眼放大区域的点 或者都不属于 ，那么 newCoord还是 fCoord
    vec2 newCoord = calcCoord(fCoord, left_eye, rmax);

    newCoord = calcCoord(newCoord, right_eye, rmax);
    //  采集到 RGBA 值
